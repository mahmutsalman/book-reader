 ...
 * Cloud-based AI service using Google's Gemini API with 1M tokens/minute free tier.
...
export class GoogleAIService implements AIServiceInterface {
  ...
  constructor(apiKey: string, model = 'gemini-2.5-flash-lite', timeout = 60000) {
    this.apiKey = apiKey;
    this.model = model;
  ...
  private isModelRateLimited(model: string): boolean {
    const cooldownExpiry = GoogleAIService.modelCooldowns.get(model);
    if (!cooldownExpiry) return false;

    if (Date.now() >= cooldownExpiry) {
      // Cooldown expired, remove from map
      GoogleAIService.modelCooldowns.delete(model);
      return false;
    }
    return true;
  }
  ...
  private markModelRateLimited(model: string): void {
    ...
    GoogleAIService.modelCooldowns.set(model, expiryTime);
    console.warn(`[Google AI] Model ${model} rate-limited, cooldown until ${new Date(expiryTime).toISOString()}`);
  }
  ...
  async testConnection(): Promise<{ success: boolean; models?: string[]; error?: string }> {
    ...
    try {
      // Test with a simple generation request instead of listing models
      ...
      return {
        success: true,
        models: FALLBACK_MODELS.slice() as unknown as string[],
      };
    } catch (error) {
      return {
        success: false,
...
If you provide multiple lines, separate them with "\\n".`;
    ...
    try {
      ...
      console.error('[Google AI] Failed to parse simpler analysis JSON:', error);
      return {
        simplerVersion: word,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          throw new Error(`Google AI rate limit exceeded for model ${model}`);
        }
        throw new Error(`Google AI API error: ${response.status} ${response.statusText} - ${errorBody}`);
      }
