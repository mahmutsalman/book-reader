/**
 * Mistral AI Service
 * Cloud-based AI service using Mistral's API with 1 billion tokens/month free tier.
...
export class MistralService implements AIServiceInterface {
  ...
  constructor(apiKey: string, model = 'mistral-small-latest', timeout = 60000) {
    this.apiKey = apiKey;
    this.model = model;
  ...
  private isModelRateLimited(model: string): boolean {
    const cooldownExpiry = MistralService.modelCooldowns.get(model);
    if (!cooldownExpiry) return false;

    if (Date.now() >= cooldownExpiry) {
      // Cooldown expired, remove from map
      MistralService.modelCooldowns.delete(model);
      return false;
    }
    return true;
  }
  ...
  private markModelRateLimited(model: string): void {
    ...
    MistralService.modelCooldowns.set(model, expiryTime);
    console.warn(`[Mistral] Model ${model} rate-limited, cooldown until ${new Date(expiryTime).toISOString()}`);
  }
...
If you provide multiple lines, separate them with "\\n".`;
    ...
    try {
      ...
      console.error('[Mistral] Failed to parse simpler analysis JSON:', error);
      return {
        simplerVersion: word,
